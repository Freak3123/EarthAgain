...
export async function POST() {
  try {
    ...
    const toDelete = await Speakers.find({ createdAt: { $lt: CUTOFF } });

    const paths = toDelete
      .map((s) => {
        const url = s.image as string | undefined;
        return url?.split("/storage/v1/object/public/speakers/")[1];
      })
      .filter((p): p is string => Boolean(p));

    if (paths.length > 0) {
      await supabase.storage.from("speakers").remove(paths);
    }

    const result = await Speakers.deleteMany({ createdAt: { $lt: CUTOFF } });

    return NextResponse.json({
      success: true,
      message: `Deleted ${result.deletedCount} speaker(s) before 2026`,
